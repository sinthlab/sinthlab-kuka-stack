...
void EffortControllerBase::updateJointStates() {
  for (size_t i = 0; i < m_joint_number; ++i) {
    ...
    // >>> SINTHLAB PATCH (see ../README.md, patch #1) — Jazzy ros2_control compatibility.
    // Upstream reads these with get_value(), which on Jazzy is get_optional().value() and THROWS
    // std::bad_optional_access while a state interface is still empty. The joint POSITION interface
    // has a URDF initial_value so it is always populated, but VELOCITY has none: it stays
    // std::nullopt until the FRI hardware read() delivers its first commanding frame. The controller
    // must be active for FRI to reach commanding, so the first update() inevitably ran against an
    // empty velocity interface -> exception -> controller deactivated ("bad optional access").
    // Reading via get_optional() and skipping the cycle until values exist makes the first few
    // updates no-ops instead of a crash. (Delaying activation cannot fix this: it is a deadlock —
