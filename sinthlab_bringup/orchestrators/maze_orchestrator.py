...
class MazeOrchestratorNode(rclpyNode):
    ...
    move_to_start -> quiet_window -> go_cue -> (maze_fixtures + checkpoint_monitor + timeout)
    -> [reward_cue per checkpoint, any order] -> goal OR timeout -> stop fixtures
    -> force_release (wait until the operator lets go) -> move_recover -> repeat.

    The operator drives the compliant arm through corridor-shaped virtual fixtures; the cabinet
    impedance provides the walls. Both the goal and the timeout converge on the same safe ending:
    stop the fixtures, wait for release, then return to start.
    """
    ...
    def on_move_complete(self):
        self.get_logger().info("Arm at maze start. Waiting for a quiet window...")
    ...
    def on_force_released(self):
        self.get_logger().info("Arm released. Returning to start.")
